function generateWorld() {
		...
		for (var i = 0; i < MAPWIDTH; i++) {
			...
			for (var j = 0; j < MAPHEIGHT; j++) {
				...
				} else if (random <= APPLETREES + TREES + BUSHES + FLOWERS + STONES) {
					obj = Crafty.e("2D, DOM, solid, Destroyable, Dropper, DamageReduction, stoneVein")
								.attr({x: i * TILESIZE, y: j * TILESIZE, z: 2000})
								.setLife(5 * Crafty.math.randomInt(2,5))
								.setDamageReduction(4)
								.setPenetrationType("Pick")
								.setNormalDrops(dropFactory.getStoneBoulder1())
								.setDamageDrops(dropFactory.getStoneBoulder2());
				}
